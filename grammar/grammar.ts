...
import { IRawGrammar, IRawRepository, IRawRule } from "../raw_grammar.ts";
import {
  IRuleFactoryHelper,
  IRuleRegistry,
  Rule,
...
} from "./basic_scopes_attribute_provider.ts";
import { _tokenizeString } from "./tokenize_string.ts";
...
function scopesAreMatching(thisScopeName: string, scopeName: string): boolean {
  ...
  return thisScopeName.length > len &&
    thisScopeName.substr(0, len) === scopeName && thisScopeName[len] === ".";
}
...
export class Grammar implements IGrammar, IRuleFactoryHelper, IOnigLib {
  ...
  private _tokenize(
    ...
    if (!prevState || prevState === StateStack.NULL) {
      isFirstLine = true;
      const rawDefaultMetadata = this._basicScopeAttributesProvider
...
export class AttributedScopeStack {
  ...
  private static _equals(
    ...
    do {
      ...
      if (
        a.scopeName !== b.scopeName || a.tokenAttributes !== b.tokenAttributes
      ) {
        return false;
      }
    ...
    if (this._emitBinaryTokens) {
      ...
      if (
        ...
        if (this.balancedBracketSelectors) {
          containsBalancedBrackets = this.balancedBracketSelectors.match(
            scopes,
          );
        }
      }
