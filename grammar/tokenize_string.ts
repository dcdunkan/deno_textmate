...
import { DebugFlags, UseOnigurumaFindOptions } from "../debug.ts";
import type { LineTokens, StateStack } from "./grammar.ts";
import {
  disposeOnigString,
  FindOption,
  IOnigCaptureIndex,
...
import { performanceNow } from "../utils.ts";
import type { AttributedScopeStack, Grammar, Injection } from "./grammar.ts";
...
function matchRule(
  ...
  if (DebugFlags.InDebugMode) {
    ...
    if (r) {
      console.log(
        `matched rule id: ${r.ruleId} from ${r.captureIndices[0].start} to ${
          r.captureIndices[0].end
        }`,
      );
    }
  }
