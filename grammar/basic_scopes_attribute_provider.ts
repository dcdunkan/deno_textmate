...
import { OptionalStandardTokenType } from "../encoded_token_attributes.ts";
import { IEmbeddedLanguagesMap } from "../mod.ts";
import { ScopeName } from "../theme.ts";
import { CachedFn, escapeRegExpCharacters } from "../utils.ts";
...
export class BasicScopeAttributesProvider {
  ...
  private readonly _embeddedLanguagesMatcher: ScopeMatcher<
    /* language id */ number
  >;
  ...
  constructor(values: [ScopeName, TValue][]) {
    if (values.length === 0) {
      ...
      const escapedScopes = values.map(
        ([scopeName]) => escapeRegExpCharacters(scopeName),
      );
