...
import { IRawGrammar, IRawRule } from "../raw_grammar.ts";
import { ScopeName } from "../theme.ts";
import { mergeObjects } from "../utils.ts";
import { IGrammarRepository } from "./grammar.ts";
...
function collectExternalReferencesInTopLevelRule(
  ...
  if (
    context.selfGrammar.patterns && Array.isArray(context.selfGrammar.patterns)
  ) {
