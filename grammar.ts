...
import { clone, mergeObjects } from "./utils.ts";
import {
  FindOption,
  IOnigCaptureIndex,
  IOnigLib,
...
function _extractIncludedScopesInPatterns(
  ...
  for (const pattern of patterns) {
    ...
    const patternRepository = (pattern.repository
      ? mergeObjects({}, repository, pattern.repository)
      : repository);
...
function collectDependencies(
  ...
  if (selfGrammar.injections) {
    const injections: IRawRule[] = [];
    for (const injection in selfGrammar.injections) {
